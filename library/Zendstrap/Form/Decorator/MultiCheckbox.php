...
class Cgmi_Form_Decorator_MultiCheckbox extends Zend_Form_Decorator_Abstract implements ZendX_JQuery_Form_Decorator_UiWidgetElementMarker
{
    ...
    public function buildMultiCheckbox()
    {
        $element = $this->getElement();
        
        $output = array();
        if (in_array($element->getType(), array('Cgmi_Form_Element_MultiCheckbox'))){
            ...
            if (count($options['options']) > 0){
                ...
                foreach ($options['options'] as $key => $value){
                    ...
                    $output[] = '<label class="checkbox-inline" for="'.$name.'-'.$nID.'">'
    ...
    public function buildErrors()
    {
        ...
        $return .= $this->buildMultiCheckbox();
        $return .= '</div>';
